...
probabiliy. To generate the Markov chain, a random number
...
a lower local enrergy \(E_L(i)\).

***** Code

#+name: getweights
#+begin_src julia
function getWeightij(ham, tau, ET, i, j,vec,elij)
    if(i!=j)
        return(1.0);
    else
        numer = (1.0 - tau * ham[i,i] + tau * ET);
        denom = (1.0 - tau * ham[i,i] + tau * elij[i]);
        if(denom==0.0)
            return typemax(Float64)
        else
            return(numer/denom)
