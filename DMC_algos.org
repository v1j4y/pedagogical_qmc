...
function do_SRDMC(ham, vec0, ET, tau, nruns, niter, nsteps, nwalker)
    ...
    walkerlisti = zeros(nwalker)
    walkerlistj = zeros(nwalker)
    for irun in 1:nruns
        # Initialize lists
        elocaltot = zeros(niter);
        wlocaltot = zeros(niter);
        ...
        for it in 1:niter
            ...
            for nw in 1:nwalker
                walkerlistj[nw] = doSwap(rand(),walkerlistj[nw],pijsummed,pijidsSorted);
        ...
        for istep in 1:nsteps
            ...
            for it in 1:niter
                ...
                wlocaltotperw = 0.0;
                for nw in 1:nwalker
                    elocaltotprew += elij[walkerlistj[nw]] * prodweightlist[nw,it];
