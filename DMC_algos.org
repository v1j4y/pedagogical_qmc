...
function do_DMC(ham, vec0, ET, tau, nruns, niter, nsteps, nwalker)
    ...
    for irun in 1:nruns
        ...
        for istep in 1:nsteps
            ...
            #println("iter --- ",istep," ----");
            #println("E= ",elocaltot);
            #println(" # 1s = ",100*sum(walkerlistj.==1)/200," ",100*sum(walkerlistj.==2)/200," ",100*sum(walkerlistj.==3)/200," ",100*sum(walkerlistj.==4)/200," ",100*sum(walkerlistj.==5)/200," ",100*sum(walkerlistj.==6)/200);
            # Perform MC step for each walker
            for nw in 1:nwalker
                j = walkerlistj[nw];
                i = walkerlisti[nw];
...
function do_SRDMC(ham, vec0, ET, tau, nruns, niter, nsteps, nwalker)
    ...
    for irun in 1:nruns
        ...
        for istep in 1:nsteps
            ...
            elocalavg = 0.0;
            for nw in 1:nwalker
                elocalavg += elij[walkerlistj[nw]];
            end
            for it in 1:niter
                elocaltot[it] += (elocalavg/nwalker) * prodweightlist[it];
                wlocaltot[it] += prodweightlist[it];
            end

            #println("iter --- ",istep," ----");
            #println("E= ",elocaltot);
            #println("E/W= ",elocaltot./wlocaltot);
            #println(" # 1s = ",100*sum(walkerlistj.==1)/200," ",100*sum(walkerlistj.==2)/200," ",100*sum(walkerlistj.==3)/200," ",100*sum(walkerlistj.==4)/200," ",100*sum(walkerlistj.==5)/200," ",100*sum(walkerlistj.==6)/200);
            # Perform MC step for each walker
            for nw in 1:nwalker
                j = walkerlistj[nw];
