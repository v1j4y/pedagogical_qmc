    ...
    for irun in 1:nruns
        ...
        for istep in 1:nsteps
            ...
            for nw in 1:nwalkerold
                ...
                elseif pspawnj > 1.0
                    walkerlistj[nwalker+1:nwalker+Int32(pspawnj)] .= jnew;
                    walkersignj[nwalker+1:nwalker+Int32(pspawnj)] .= sgnj;
                    nwalker = nwalker + Int32(pspawnj);
                    if (pspawnj-Int32(pspawnj)) > rand()
                        nwalker = nwalker + 1;
                        walkerlistj[nwalker] = jnew;
                        walkersignj[nwalker] = sgnj;
                    end
        ...
        # Fill global list
        #localenergy[irun] = ET;
        n0 = size(findall(x->abs(abs(x)-1.0) < 1e-4,walkerlistj))[1];
        n1 = size(findall(x->abs(abs(x)-2.0) < 1e-4,walkerlistj))[1];
        n2 = size(findall(x->abs(abs(x)-3.0) < 1e-4,walkerlistj))[1];
        println(n0," ",n1," ",n2);
        println(walkersigni[1],"\t",walkersigni[n0+1],"\t",walkersigni[n0+n1+1]);
        localenergy[irun] = ham[1,2] * n1/n0 + ham[1,3] * n2/n0;
    end
