...
#+name: calc_local_kenergy_fciqmc
#+begin_src julia :tangle srdmc.jl
function getKE_fciqmc(ham, vi, i)
    return(getEL_fciqmc(ham,vi,i) - ham[i,i])
end
#+end_src

#+name: calc_trap_time
#+begin_src julia :tangle srdmc.jl
function getTraptime_fciqmc(ham, vi, i)
    TL = getKE_fciqmc(ham, vi, i);
    return(-1.0/TL)
end
#+end_src

#+name: prepare_pij_list_fciqmc
#+begin_src julia :tangle srdmc.jl
...
#+name: getweights_fciqmc
#+begin_src julia :tangle srdmc.jl
function getWeightij_fciqmc(ham, tau, ET, i, j,ELi)
    if (i==j)
        TLi = ELi - ham[i,i];
        return(TLi/(ET - ham[i,i]))
    else
...
#+begin_src julia :tangle srdmc.jl
function prepare_pij_wij_fciqmc(ham, vec, ET, tau)
    dim = size(ham)[1];
    pij = SortedDict{Any, Any}[SortedDict() for i = 1:dim];
    countswaps = zeros(dim);
    elij = Dict();
    ...
    for i in 1:dim
        for j in 1:dim
            EL = getEL_fciqmc(ham, vec, i)
            EL1 = EL;
            θi = getTraptime_fciqmc(ham, vec, i)
            w12 = getWeightij_fciqmc(ham, tau, ET, i, j, EL);
            if i == j
                p12 = 0.0
            else
...
#+begin_src julia :tangle srdmc.jl
function do_FCIQMC(ham, vec0, ET, tau, nruns, niter, nsteps)
    pijsummed, pijidsSorted, elij, weightij, thetaij = prepare_pij_wij_fciqmc(ham, vec0, ET, tau);
    localenergy = zeros(nruns, niter);
    weightlist = zeros(niter);
    prodweightlist = zeros(niter);
    #prodweightlist_per_i = zeros(niter);
    ...
    for irun in 1:nruns
        ...
        for it in 1:niter
            # Perform MC step
            j = doSwap_fciqmc(rand(),j,pijsummed,pijidsSorted);
        ...
        for istep in 1:nsteps
            ...
            # Perform MC step
            jnew = doSwap_fciqmc(rand(),j,pijsummed,pijidsSorted);
            j = jnew;
...
localenergypdmc = do_PDMC(ham, vec0, ET, tau, nruns, niter, nsteps);
localenergy = do_FCIQMC(ham, vec0, ET, tau, nruns, niter, nsteps);
for it in 1:niter
...
hline!([-0.99],line=:dash,label=nothing)
hline!([-0.21432],line=:dash,label=nothing)
hline!([0.381966],line=:dash,label=nothing)
hline!([-2.366],line=:dash,label=nothing)
figpath="/tmp/";
