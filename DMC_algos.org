...
***** DMC Special functions

#+name: prepare_pij_lists_with_weights
#+begin_src julia :tangle srdmc.jl
function prepare_pij_wij_dmc(ham, vec, ET, tau)
    dim = size(ham)[1];
    pij = SortedDict{Any, Any}[SortedDict() for i = 1:dim];
    countswaps = zeros(dim);
    elij = Dict();
...
#+name: prepare_pij_list_dmc
#+begin_src julia :tangle srdmc.jl
function getPij_dmc(ham, vec, ET, tau, pijsummed)
    dim = size(ham)[1];
    pij = zeros(dim,dim);
    for k in 1:dim
        ELk = getEL(ham,vec,k);
        for l in 1:dim
            if k!=l
                #pij[k,l] = tau*getWeightij(ham,tau,ET,k,l,ELk)/pijsummed[k][l];
                pij[k,l] = getWeightij(ham,tau,ET,k,l,ELk);
            end
        end
    end
    return(pij);
...
#+name: prepare_pij_list
#+begin_src julia :tangle srdmc.jl
function getPii_diag(ham, vec, ET, tau)
    dim = size(ham)[1];
    pijdiag = zeros(dim);
    idmat = Matrix(1.0*I,dim,dim);
    for k in 1:dim
        #ELk = getEL(ham,vec,k);
        #pijdiag[k] = getWeightij(ham,tau,ET,k,k,ELk)
        pijdiag[k] = tau * (ham[k,k] - ET);
    end
    return(pijdiag);
end
#+end_src

#+name: adjustS_states_dmc
#+begin_src julia :tangle srdmc.jl
function adjustS_dmc(Sold,ζ,tau,Nw,Nwold,stepsize)
    if Nw == 0
        return(Sold + 0.0001)
    end
...
#+name: dmc_algo
#+begin_src julia :tangle srdmc.jl
function do_DMC(ham, vec0, ET, tau, stepsize, ζ, nruns, nsteps, nwalkermax)
    dimH = size(ham)[1];
    pijsummed, pijidsSorted, elij, weightij = prepare_pij_wij_dmc(ham, vec0, ET, tau);
    pijdiag = getPii_diag(ham, vec0, ET, tau);
    pijfull = getPij_dmc(ham, vec0, ET, tau, pijsummed);
    localenergy = zeros(nruns);
    walkerlisti = zeros(Int64,nwalkermax);
    walkerlistj = zeros(Int64,nwalkermax);
    weightlistj = zeros(Float64,nwalkermax);
    totalwalkerlist = zeros(Int32,nsteps);
    etlist = zeros(Float64,nsteps);
    nreconfmean = 0.0;
    nreconftot = 0;
    ...
    for irun in 1:nruns
        ...
        for istep in 1:nsteps
            ...
            for nw in 1:nwalkerold
                elocaltot += elij[walkerlisti[nw]]/nwalkerold;
            end

            # Perform MC step for each walker
            for nw in 1:nwalkerold
                i = walkerlisti[nw];
                jnew = doSwap(rand(),i,pijsummed,pijidsSorted);
                probspawn = pijfull[i,jnew] * 1/4;
                #if intprobspawn > 0
                #    walkerlistj[nwalker+1:nwalker+intprobspawn] .= jnew;
                #    nwalker = nwalker + intprobspawn;
                #    if rand() < realprobspawn
                #        nwalker = nwalker + 1;
            ...
            nwspawn = nwalker;
            #println("nspawn=",nwspawn)

            ## Branching

            #nwalker, nnewwalkers = branch_dmc(weightlistj, walkerlistj, nwalker);
            ...
            nwcloned = 0
            for nw in 1:nwalkerold
                pidiag = pijdiag[walkerlisti[nw]];
                #intpidiag = Int(floor(pidiag)) + rand();
                realpidiag = pidiag;
                #if intpidiag > 0
                #    walkerlistj[nwalker+1:nwalker+intpidiag] .= walkerlisti[nw];
                #    nwalker = nwalker + intpidiag;
                #    nwcloned = nwcloned + intpidiag;
                #    if rand() > realpidiag
                #        nwalker = nwalker + 1;
                #        nwcloned = nwcloned + 1;
                #        walkerlistj[nwalker] = walkerlisti[nw];
                #    end
                #else
                if realpidiag > 0
                    if rand() < abs(realpidiag)
                        nwdead = nwdead + 1;
                        walkerlisti[nw] *= -1.0;
                    end
                end
            ...
            #println("\tncloned=",nwcloned)
            #println("ndead=",nwdead)

            # Prune dead walkers

            walkerlisti_nodead = walkerlisti[findall(x->x>0,walkerlisti[1:nwalkerold])];
            nwalkerold = size(walkerlisti_nodead,1);
            walkerlisti[1:nwalkerold] = walkerlisti_nodead;

            # Merge
            walkerlist_new = vcat((walkerlisti[1:nwalkerold],walkerlistj[1:nwalker])...);
            nwalkerold = size(walkerlist_new,1);
            walkerlisti[1:nwalkerold] = walkerlist_new;

            Nw = nwalkerold;
            #println("--- nw = ",Nw," --- ",nwalker)
            ...
            if istep % stepsize == 0 && doStep
                #println(" ndead= ",nwdead," ncloned= ",nwcloned," nspawn= ",nwspawn);
                # Change ET
                if istep % (stepsize * 5) == 0
                    println("istep = ",istep, " Nw = ",Nw," Nwold = ",Nwold," ET = ",ET," ζ= ",ζ," stepsize= ",stepsize," tau= ",tau);
                    println(numlist)
                end
                ET = adjustS_dmc(ET,ζ,tau,Nw,Nwold,stepsize);
                pijsummed, pijidsSorted, elij, weightij = prepare_pij_wij_dmc(ham, vec0, ET, tau);
                pijdiag = getPii_diag(ham, vec0, ET, tau);
                pijfull = getPij_dmc(ham, vec0, ET, tau, pijsummed);
                for idx in 1:dimH
                    numlist[idx] = count(x->x==idx,walkerlisti[1:nwalkerold])
                end
                #println(pijdiag .- 1)
                #println(pijfull)
            end
...
#+name: branching_dmc
#+begin_src julia :tangle srdmc.jl
function branch_dmc(weightlist, walkerlist, nwalker)
    # Make new walker list
    nnewwalkers = 0;
    for nw in 1:nwalker
        intweight = Int(floor(weightlist[nw]));
        restweight = weightlist[nw] - intweight;
        if intweight < 1.0
            if rand() < restweight
                nwalker += 1;
                nnewwalkers += 1;
                walkerlist[nwalker] = walkerlist[nw];
            end
        else
            walkerlist[nwalker+1:nwalker+intweight] .= walkerlist[nw];
            nwalker += intweight;
            nnewwalkers += intweight;
            if rand() < restweight
                nwalker += 1;
                nnewwalkers += 1;
                walkerlist[nwalker] = walkerlist[nw];
...
function do_SRDMC(ham, vec0, ET, tau, nruns, niter, nsteps, nwalker)
    ...
    for irun in 1:nruns
        ...
        for istep in 1:nsteps
            ...
            newwalkerlistj = branch_srdmc(problistj[1:nreconfig], walkerlistj[idbig], nreconfig);
...
#+name: branching
#+begin_src julia :tangle srdmc.jl
function branch_srdmc(problist,   walkerlist,nwalker)
    # Make new walker list
...
localenergypdmc, countimespdmc = do_PDMC(ham, vec0, ET, tau, nruns, niter, nsteps);
localenergy,totalwalkerlist,etlist = do_DMC(ham, vec0, ET, tau, stepsize, ζ, nruns, nsteps, nwalkermax);
...
hline!([-0.877058],line=:dash,label=nothing)
