...
function do_SRDMC(ham, vec0, ET, tau, nruns, niter, nsteps, nwalker)
    ...
    weightlist = zeros(Float64,nwalker);
    weightlistscaled = zeros(Float64,nwalker);
    globalweightlist = zeros(Float64,niter);
    problistj = zeros(Float64,nwalker);
    for irun in 1:nruns
        # Initialize lists
        j = 1;
        ...
        for it in 1:niter
            ...
            # Initialize weightlist
            globalweightlist[it] = weightperwalker/nwalker;
        end

        # Prepare prodweightlist
        for it in niter:-1:1
            prodweightlist[niter-it+1] = wlocal * globalweightlist[it];
            wlocal = wlocal * globalweightlist[it];
        end

        # Do DMC
        elocaltot = zeros(Float64,niter);
        wlocaltot = zeros(Float64,niter);
        elocaltotwalker = 0.0;
        for istep in 1:(Int(nsteps/nsrsteps))

            # Calculate elocal and wlocal
            if elocaltotwalker != 0.0
                for it in 1:niter
                    elocaltot[it] += elocaltotwalker * prodweightlist[it]/nsrsteps;
                    wlocaltot[it] += prodweightlist[it];
                end
            end
            ...
            elocaltotwalker = 0.0;
            for srstep in 1:nsrsteps
                # Calculate elocal
                #elocaltotwalker = 0.0;
                for nw in 1:nwalker
                    elocaltotwalker += elij[walkerlistj[nw]]/nwalker;
                end

                #println("E= ",elocaltot);
                #println("wlocal= ",wlocaltot);
                #println("prodweigh= ",prodweightlist[1:10]);
                # Perform MC step for each walker
                for nw in 1:nwalker
                    j = walkerlistj[nw];
                    i = walkerlisti[nw];
                ...
                if srstep == nsrsteps
                    ...
                    for it in 1:niter-1
                        ...
                        wprev = globalweightlist[idxdel];
                        wnew  = wlocal;
                        prodweightlist[it] = prodweightlist[it] * wnew/wprev;
                    end
                    idxnew = (((istep-1) % niter) + 1);
                    wprev = globalweightlist[idxnew];
                    globalweightlist[idxnew] = wlocal;
                    prodweightlist[niter] = prodweightlist[niter] * wlocal/wprev;
            ...
            #println("prodlist= ",prodweightlist[1:4]);
            #println("eloctot= ",elocaltot[1:4]);
            #println("wloctot= ",wlocaltot[1:4]);
        end
