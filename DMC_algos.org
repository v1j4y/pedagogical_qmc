...
#+name: prepare_pij_list
#+begin_src julia :tangle srdmc.jl
function getPij_diag(ham, vec, ET, tau)
    dim = size(ham)[1];
    pijdiag = zeros(dim);
    idmat = Matrix(1.0*I,dim,dim);
    for k in 1:dim
        pijdiag[k] = (1 - tau*(ham[k,k] - ET));
    end
    return(pijdiag);
end
#+end_src
...
function do_DMC(ham, vec0, ET, tau, stepsize, ζ, nruns, nsteps, nwalkermax)
    dimH = size(ham)[1];
    pijsummed, pijidsSorted, elij, weightij = prepare_pij_wij(ham, vec0, ET, tau);
    pijdiag = getPij_diag(ham, vec0, ET, tau);
    localenergy = zeros(nruns);
    ...
    for irun in 1:nruns
        ...
        for istep in 1:nsteps
            ...
            for nw in 1:nwalkerold
                ...
                weightlistj[nwalker] = weightij[(i,j)];
            end

            # Branching

            nnewwalkers = branch_dmc(weightlistj, walkerlistj, nwalker);

            # Diagonal death/cloning

            for nw in 1:nwalkerold
                if rand() > pijdiag[walkerlisti[nw]]
                    nwalker += 1;
            ...
            # Merge
            walkerlisti = vcat((walkerlisti[1:nwalkerold],walkerlistj[1:nwalker])...);
            nwalkerold = size(walkerlisti,1);
            ...
            if istep % stepsize == 0 && doStep
                ...
                if istep % (stepsize * 10) == 0
                    println("istep = ",istep, " Nw = ",Nw," Nwold = ",nwalkerold," ET = ",ET," ζ= ",ζ," stepsize= ",stepsize," tau= ",tau);
                end
