...
for i in 1:9
    ...
    eavg = 0.0;
    count = 0;
    for j in 1:shft:1024-shft
        eavg = eavg + listEL[j]*foldl(*,listwij[j:j+shft]);
        count += foldl(*,listwij[j:j+shft]);
    end
    push!(outputdata,eavg/count)
end
#end
savefig(
    plot(1:length(outputdata),outputdata,
