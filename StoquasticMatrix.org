...
for irun in 1:nruns
    ...
    for p in 1:nsteps
        ...
        circshift!(tmpenergy,energylist[irun,:],-1);
        energylist[irun,:]= tmpenergy;
        # Add new energy
        energylist[irun,end]= elij[j];

        # calculate the weighted eneries
        map(x->totalEW[irun,x]+= energylist[irun,x]*timesweight[irun,x],collect(1:niter));

        # calculate the sum of weights
        map(x->totalweight[irun,x]+= timesweight[irun,x],collect(1:niter));

        # rotate weight window
        circshift!(tmpweight,weightdata[irun,:],-1);
        weightdata[irun,:]= tmpweight;
        weightdata[irun,1]= 1;
        # Add new weight
        ...
        # calculate the sum of product of weights
        for idxi in 2:niter
            timesweight[irun,idxi] = timesweight[irun,idxi]*weightdata[irun,idxi]/oldweightdata[irun,2]
        end

        # set old weights
        oldweightdata[irun,:] = weightdata[irun,:]
...
tmpa = zeros(2,10)
tmpb = ones(10)*2
tmpa[1,1:10] = range(1,10,length=10)
function tmpfun(x,tmpvec)
    y=convert(Int,x)
    tmpvec[2,y]=foldl(*,tmpvec[1,1:y])
    return(y)
end
map(x->tmpa[2,x]=foldl(*,tmpa[1,1:x]),collect(1:10))
map(x->tmpa[1,x]*=tmpb[x],collect(1:10))
1.3^100
#+end_src
